...
const addWatchedSchema = z.object({
  mediaId: z.number(),
...
export class WatchedController {
  async add(req: Request, res: Response) {
    try {
      ...
      const item = await watchedService.addToWatched({ userId, ...data });
  ...
  async remove(req: Request, res: Response) {
    try {
      ...
      await watchedService.removeFromWatched(userId, id);
  ...
  async removeByMedia(req: Request, res: Response) {
    try {
      ...
      await watchedService.removeByMediaId(userId, mediaId, mediaType);
      res.json({ success: true });
    } catch (error: any) {
      res.status(404).json({ success: false, error: error.message || 'Not found' });
    }
  ...
  async getAll(req: Request, res: Response) {
    try {
      ...
      const result = await watchedService.getUserWatched(userId, page, limit, mediaType);
  ...
  async checkStatus(req: Request, res: Response) {
    try {
      ...
      const watchedIds = await watchedService.checkWatchedStatus(userId, mediaIds, mediaType);
  ...
  async getRecent(req: Request, res: Response) {
    try {
      const userId = (req as any).user.id;
      const items = await watchedService.getRecentWatched(userId);
  ...
  async getCounts(req: Request, res: Response) {
    try {
      const userId = (req as any).user.id;
      const counts = await watchedService.getWatchedCounts(userId);
...
export const watchedController = new WatchedController();
